...
function StatsCards({ tickets }: { tickets: import("../../features/ticket/useTickets").Ticket[] }) {
    ...
    // Only show main statuses: Open, In Progress, Waiting, On Hold, Resolved
    const mainStatusIds = [1, 2, 3, 4, 5];
    const statusCounts = mainStatusIds.map((id) => ({
        id,
        label: statuses.find((s) => s.id === id)?.name || `Status ${id}`,
        color: statuses.find((s) => s.id === id)?.color || "gray",
        count: tickets.filter((t) => t.statusId === id).length,
    }));
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-6">
            {statusCounts.map((s) => (
                <div key={s.id} className="flex items-center bg-white dark:bg-gray-900 rounded-lg shadow p-4">
                    <span className={`w-3 h-3 rounded-full mr-3 bg-${s.color}-500 dark:bg-${s.color}-600`}></span>
                    <div>
...
function PriorityChart({ tickets }: { tickets: import("../../features/ticket/useTickets").Ticket[] }) {
    ...
    const counts = priorities.map((p) => ({
        ...
        color: priorityDotColors[p.id]?.bg || "bg-gray-400",
    }));
...
function RecentActivity({ tickets }: { tickets: import("../../features/ticket/useTickets").Ticket[] }) {
    const { statuses } = useStatuses();
    const getStatus = (id: number) => statuses.find((s) => s.id === id)?.name || `Status ${id}`;
